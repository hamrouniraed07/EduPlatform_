...
router.post('/analyze-reviews/:courseId', protect, async (req, res) => {
  try {
...
// @desc    Générer une description de cours avec l'IA
...
// @desc    Suggérer des cours similaires avec l'IA
// @route   POST /api/ai/similar-courses/:courseId
// @access  Public
router.post('/similar-courses/:courseId', async (req, res) => {
  try {
    const { courseId } = req.params;
...
// @desc    Générer une bio professionnelle avec l'IA
...
La bio doit être écrite à la première personne et donner envie de connecter avec cette personne.`;
    ...
    res.status(500).json({ 
      message: 'Erreur lors de la génération de la bio',
      error: error.message 
    });
  }
});

// @desc    Obtenir des insights globaux sur la plateforme
// @route   GET /api/ai/platform-insights
// @access  Private (Admin)
router.get('/platform-insights', protect, async (req, res) => {
  try {
